...
## 📍 当前位置
- **阶段**：阶段一 · 第 2 周（进行中）
- **已完成**：第 1 周全部 ✅；第 2 周——Maven/pom.xml ✅、第一个 Spring Boot 接口 `/hello` 跑通 ✅（浏览器亲验）
- **下一步**：让接口**返回 JSON**（前端最熟的格式）+ 学会**接收参数**（`@RequestParam` / 路径参数），为第 3 周 CRUD 铺路
...
- [~] 第 2 周：第一个 Spring Boot 接口 → 产出：`/hello` 跑通 ✅
...
- **写接口的注解**（`@RestController` + `@GetMapping("/路径")`）还需反复练形成肌肉记忆——今天漏了几次靠提示补上
- **接口返回 JSON**：目前只会返回纯文本 String，还没返回 JSON（下次重点）
